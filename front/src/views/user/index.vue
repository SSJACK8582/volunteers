  ...
  <div>
    ...
    >
      ...
      <el-table-column prop="userStatus" label="状态" align="center">
        <template slot-scope="scope">
          <el-tag>{{ statusDict[scope.row.userStatus] }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="openForm">
      <el-form :inline="true" :model="dialogForm">
        ...
        <el-form-item label="状态" label-width="100px">
          <el-radio-group v-model="dialogForm.userStatus">
        ...
        <el-form-item label="用户类型" label-width="100px">
          <el-select v-model="dialogForm.userType" placeholder="用户类型">
            <template v-for="(value, key, index) of typeDict">
              <el-option :label="value" :value="key" />
            </template>
          </el-select>
  ...
  methods: {
    ...
    getDict() {
      return this.axios({
        method: "post",
        url: this.url + "/dict",
        data: ["sex", "user_type", "user_status"],
      }).then((resp) => {
        this.sexDict = resp.data.data.sex;
        this.typeDict = resp.data.data.user_type;
        this.statusDict = resp.data.data.user_status;
      });
    },
    getTableData() {
      return this.axios({
    ...
    deleteUser() {
      return this.axios({
        method: "delete",
        url: this.url + "/user/" + this.userIds,
      })
        .then((resp) => {
          if (resp.data.code == 200) {
            this.successMsg();
    ...
    resetPassword() {
      return this.axios({
        method: "put",
        url: this.url + "/user/password/" + this.dialogForm.userId,
      })
        .then((resp) => {
          if (resp.data.code == 200) {
            this.successMsg();
